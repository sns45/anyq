/**
 * SQS Tester - Hono Server
 * Test application for @anyq/sqs
 */
...
app.get('/', (c) => {
  return c.json({
    name: '@anyq/sqs-tester',
    version: '0.0.1',
    description: 'Test application for @anyq/sqs',
    endpoints: {
      '/': 'Service info',
      '/health': 'Health check',
...
async function start() {
  try {
    console.log('Starting SQS Tester...');
    console.log(`SQS endpoint: ${config.sqs.endpoint}`);
    console.log(`Queue URL: ${config.sqs.queueUrl}`);
