/**
 * @fileoverview Producer setup for memory tester
 */

import { MemoryProducer } from '@anyq/memory';
import { queueConfig } from './config.js';
import type { OrderMessage } from './types.js';

let producer: MemoryProducer<OrderMessage> | null = null;
let messagesPublished = 0;

export async function initProducer(): Promise<MemoryProducer<OrderMessage>> {
  if (producer) {
    return producer;
  }

  producer = new MemoryProducer<OrderMessage>(queueConfig);
  await producer.connect();
  console.log('✅ Producer connected');
  return producer;
}

export function getProducer(): MemoryProducer<OrderMessage> | null {
  return producer;
}
